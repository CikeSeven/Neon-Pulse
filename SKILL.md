...
**重要**：游戏设定中玩家一直在向右移动（虽然代码中玩家位置相对固定），通过以下方式营造这种效果：

- **敌人**：从右侧生成，向左移动
- **掉落物**（经验、金币等）：掉落后会持续向左移动
- **背景粒子**：向左飘动

**配置参数**：
```gdscript
# Global.gd
var world_scroll_speed: float = 100.0  # 掉落物向左移动速度

# 掉落物脚本中
@export var world_scroll_speed: float = 100.0
# 在 _physics_process 中：
velocity.x = -world_scroll_speed  # 向左移动
```
